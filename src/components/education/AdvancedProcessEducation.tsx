...
export const equipmentImages:Record<ProcessId,string>={wafer:waferImage,oxidation:oxidationImage,lithography:lithographyImage,etch:etchImage,deposition:depositionImage,metallization:metallizationImage,eds:edsImage,packaging:packagingImage};
export const hotspotPositions:Record<ProcessId,{x:number;y:number}[]>={
  ...
  return <div className="flex-1 overflow-y-auto rounded-xl border border-slate-800 bg-[#030712] p-5">
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[1.3fr_1fr]">
      <div className="equipment-3d-stage min-h-[520px]">
        <img key={"structure-" + processId} loading="eager" fetchPriority="high" src={equipmentImages[processId] + "?v=20260819-3"} alt={p.equipment + " 실사형 내부 구조"} className="absolute inset-0 z-0 block h-full w-full object-contain"/>
        <div className="pointer-events-none absolute inset-0 z-[1] bg-gradient-to-t from-black/55 via-transparent to-white/5"/>
